...
Your choice is: add
```
执行`select`语句时，会列出序号`1～n`标记的菜单，序号与`in`之后给定的字(word)对应，然后给出提示，并接受用户的选择，并将该数据赋给环境变量`REPLY`。如果输入的数据是`1～n`中的一个值，那么参数`identifier`（本例中为choice）就置为该数字对应的字。如果未输入数据，则重新显示该菜单，该参数设置为`null`。对于每一个选择执行`do-done`的命令行，直至遇到`break`或者文件结束标志。
